...
- `gguf_dequantization`: upstream GGML's fp32 dequantization of the reference
  fixture of each GGUF format (`gguf-repack`). They pin the CPU reference
  (`dev/tests/engine/GgufFormatReference.hpp`) to llama.cpp 7ab4ee7, so no
  Splash change touches them.
- `gguf_images`: every image `gguf-repack` prepares from its dense and
  MoE GGUF fixtures.
...
1. Give the new bytes a new preparation identity, or caches of the old bytes
   ...
   `INPUTS`.
2. Run the tests: `gguf-repack` prints each mismatching image with its
...
3. Check that only the images the change should touch moved and that the
   ...
   `gguf-repack`, the serialized `expected` images in
...
`SPLASH_GGML_ORACLE=<libggml-base.dylib> build/engine-tests/gguf-repack --cpu dev/tests/fixtures/weight-goldens/goldens.json`
